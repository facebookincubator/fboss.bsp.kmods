...
#define DRIVER_NAME	"icetea_fancpld"

/* Fan PWN is defined as 0 to 64 mapped to 0 to 100%, multiplied by 1.5625% */
...
static int mcbfan_fan_input_read(struct device *dev, int channel, long *val)
{
	...
	fn = channel + 1;
	reg_addr = FBMCB_REG_FAN_TACH_F_N(fn);
...
static const struct hwmon_channel_info *mcbfan_info[] = {
	HWMON_CHANNEL_INFO(fan,  // 4 fans x 1 tach (Front)
...
/* MCBCPLD_FAN id */
static const struct i2c_device_id icetea_fancpld_i2c_id[] = {
	{ DRIVER_NAME, 0 },
	{ },
};
MODULE_DEVICE_TABLE(i2c, icetea_fancpld_i2c_id);
...
static struct i2c_driver icetea_fancpld_i2c_driver = {
	...
	.id_table = icetea_fancpld_i2c_id,
};

module_i2c_driver(icetea_fancpld_i2c_driver);

MODULE_AUTHOR("Joy Wu <joy.wu@celestica.com>");
MODULE_DESCRIPTION("Meta FBOSS MCBCPLD (FAN) Driver");
MODULE_LICENSE("GPL");
