 ...
 */
#if IS_ENABLED(CONFIG_SENSORS_MPQ8785)

#include "dummy_kmod.h"
create_dummy_kmod(mpq8785);

#else /* !CONFIG_SENSORS_MPQ8785 */
...
static int mpq8785_probe(struct i2c_client *client)
{
	...
	if (dev->of_node) {
		...
		const struct i2c_device_id *id = i2c_match_id(mpq8785_id, client);
		if (!id)
			return -ENODEV;
